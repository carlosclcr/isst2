...
public class FormDefinitivaServlet extends HttpServlet {
  ...
  Acta acta = null;
	  try { acta = client.target(URLHelper.getURL() + "/" + id)
	  ...
	  if (acta != null) {
	      ...
	      case "2" : 
              ...
              acta.setStatus(2);
              enviaCorreo("destino", 1, asig, Double.parseDouble(req.getParameter("nota")));
              client.target(URLHelper.getURL()+ "/" + id).request()
	           .post(Entity.entity(acta, MediaType.APPLICATION_JSON), Response.class);
		...
		default:
		  ...
		  List<Acta> actas = client.target(URLHelper.getURL() + "/professor/"+ acta.getEmail_coordinador())
...
req.setAttribute("actas", actas);
	  ...
	  Asignatura asignatura = client.target(URLHelper.getURL_asignatura() + "/professor/" + acta.getEmail_coordinador()).request()
...
getServletContext().getRequestDispatcher("/Professor.jsp").forward(req,resp);
	...
	public void enviaCorreo(String destino,int nivel, String asignatura, double nota) {
		...
		String mail = "c.lopezdeceballos@alumnos.upm.es";
		String asunto = "Publicación de notas definitivas: " + asignatura ;
		String cuerpo = "Le comunicamos que su nota provisional en la asignatura de "
+ asignatura + " es de un " + nota + ".";
		...
		try {
			System.out.println("Se enviara el correo?");
